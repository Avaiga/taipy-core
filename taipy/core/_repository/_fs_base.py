...
class _FileSystemRepository(Generic[ModelType, Entity]):
    ...
    @abstractmethod
    def _from_model(self, model, org_entity=None, lazy_loading=True):
        """
    ...
    def load(self, model_id: str, entity: Entity = None, lazy_loading: bool = True) -> Entity:
        return self.__to_entity(self.__get_model_filepath(model_id), entity=entity, lazy_loading=lazy_loading)
    ...
    def __to_entity(self, filepath, entity: Entity = None, lazy_loading: bool = True):
        with open(filepath, "r") as f:
            data = json.load(f, cls=_CustomDecoder)
        model = self.model.from_dict(data)  # type: ignore
        return self._from_model(model, entity, lazy_loading)
